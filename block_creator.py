...
def message_to_blocks(message,version,ec_level):
    global BLOCK_INFORMATION
    block_format = BLOCK_INFORMATION[version][ec_level]
    reader_index = 0
    blocks = []
    for i in range(block_format[0]):
        blocks.append(message[reader_index:reader_index + block_format[1]])
        reader_index += block_format[1]

    for i in range(block_format[2]):
        blocks.append(message[reader_index:reader_index + block_format[3]])
        reader_index += block_format[3]
